...
// --- Handlers de Adição, Edição e Exclusão ---

func AddAtivoNacional(c *gin.Context) {
    userID := c.MustGet("userID").(int64)
    
    // CORREÇÃO: Adicionado o tipo 'AddNacionalPayload' que estava faltando
    ...
    // Métrica OK
    middleware.InvestmentsCreated.WithLabelValues("nacional").Inc()
...
func AddAtivoInternacional(c *gin.Context) {
    ...
    // Métrica OK
    middleware.InvestmentsCreated.WithLabelValues("internacional").Inc()
